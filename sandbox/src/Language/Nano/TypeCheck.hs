...
instance HasTVars Type where
  ...
  freeTVars (Forall as t)   = freeTVars t L.\\ as -- taken from UCSC slides
  freeTVars _               = []
...
unify st (a :=> b)  (c :=> d)              = unify (unify st (apply (stSub st) a) (apply (stSub st) c)) (apply (stSub st) b) (apply (stSub st) d)
...
-- infer st gamma (EVar x)        = helper st gamma x
--  where
--    helper a b c =
--      case (lookupVarType x gamma) of
--        (Mono )         =
--        _               = error "TBD: infer polys on evar"
...
generalize gamma t = Forall uvs t
  where
    tvs   = freeTVars t
    gvs   = freeTVars gamma
    uvs   = L.nub (tvs L.\\ gvs)
