...
pipe fs   = foldLeft f base fs
  where
    f a x = a.x -- credit: Piazza @470
    base  = \id -> id -- credit: Piazza @491
...
padZero l1 l2
        | size1 == size2  = (l1, l2)
        | size1 > size2   = (l1, (clone 0 (size1 - size2)) ++ l2)
        | otherwise       = ((clone 0 (size2 - size1)) ++ l1, l2)
  where 
    size1 = length l1
    size2 = length l2
...
bigAdd l1 l2     = removeZero res
  where
    ...
    f (carry:sum) (x,y) = [div digitSum 10] ++ [mod digitSum 10] ++ sum
      where
        digitSum = x + y + carry
    base         = [0]
...
bigMul l1 l2 = res
  where
    ...
    f (pad, finalProduct) x = (pad + 1, bigAdd paddedProduct finalProduct)
      where 
        digitProduct = mulByDigit x l2
        paddedProduct = digitProduct ++ clone 0 pad
    base = (0, [])
    args = reverse l1
